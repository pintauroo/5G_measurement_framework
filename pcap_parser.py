...
from helper.util import print_line, open_compressed_file, colorize
from helper.csv_writer import write_to_csv
import pandas as pd
...
flags = {10000: False, 10001: False, 10002: False}  # to find control ports
control_ports=[]
tuple_global=[]
...
def main():
    ...
    for ts, buf in pcap:
        ...
        if src_port > dst_port:
            ...
            if tcp_tuple not in tuple_global:
                tuple_global.append(tcp_tuple)
        else:
...
# ---------------------------------------------------------------
    ...
    p = PcapData(   
                    ...
                    cwnd_values=cwnd_values if cwnd_values is not None else None,
                    retransmissions=retransmissions,
                    retransmissions_interval=retransmissions_interval,
                    data_info=data_info if bbr_values is not None else None,
                    wireless_data=wireless_data if wireless_data is not None else None
                )
    p = plot_all('.', p, plot_only=['wireless', 'cwnd', 'sending_rate', 'throughput', 'fairness', 'rtt', 'avg_rtt', 'inflight'])
...
def parse_bbr_and_cwnd_values():
    ...
    for line in lines:    
        ...
        for p in control_ports:
            updated_line = line.replace("rcv_space:"+str(p), "")
            if str(p) in updated_line:
                flag = True
                continue
        ...
        if "ESTAB" in line:
            ...
            for t in tuple_global: 
                if src_addr in t and int(src_port) in t and dst_addr in t and int(dst_port) in t:
                    tcp_tuple = t


        elif tcp_tuple is not None:
            if data_table[tuple(tcp_tuple)]['cc'] == '':
                for ccc in ['reno', 'cubic', 'bbr', 'New Reno', 'BIC', 'DCTCP', 'Compound', 'TCP Vegas']:
                    if ccc in line:
                        data_table[tuple(tcp_tuple)]['cc'] = ccc
                        continue
            ...
            if 'timestamp' in fields: 
                data_table[tuple(tcp_tuple)]['data'].append(fields)

                ts = timestamp.timestamp_to_unix_time(fields['timestamp'], timestamp_local)
                data_table[tuple(tcp_tuple)]['cwnd_values'][0].append(float(ts) if 'timestamp' in fields else 0)
                data_table[tuple(tcp_tuple)]['cwnd_values'][1].append(float(fields['cwnd']) if 'cwnd' in fields else 0)
                data_table[tuple(tcp_tuple)]['cwnd_values'][2].append(float(fields['ssthresh']) if 'ssthresh' in fields else 0)
...
def wireless_parser(wireless_data):
    data = pd.read_csv(wireless_data)
    ts = data['tmbuf']
    rsrp = data['avg_rsrp']
    if 'DL' in sys.argv[5]:
        bler = data['dl_bler_stats.bler']
        mcs = data['dl_bler_stats.mcs']
    elif 'UL' in sys.argv[5]:
        bler = data['ul_bler_stats.bler']
        mcs = data['ul_bler_stats.mcs']
    ...
    for i, t in enumerate(ts):
        if str(t) != 'nan' and str(t) != ' ':
            unix_ts.append(timestamp.timestamp_to_unix_time(str(t), timestamp_local))
            rsrp_.append(rsrp[i])
            bler_.append(bler[i])
            mcs_.append(mcs[i])
    return [unix_ts, rsrp_, bler_, mcs_]
