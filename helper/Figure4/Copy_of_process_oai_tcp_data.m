...
items = dir(main_folder);

folders = {}
% Iterate through the items and print folder names
for i = 1:numel(items)
    if items(i).isdir && ~strcmp(items(i).name, '.') && ~strcmp(items(i).name, '..')
        if contains(items(i).name, 'UL_24')
            folders = [folders, items(i).name];
...
for i = 1:numel(folders)
    disp(folders(i));  % Display each element individually
end
