...
def write_to_csv(path, pcap_data, compression):
    ...
    for value in value_dict:
        if value:
            if value =='cwnd_values' :
                # continue
                cwnd = {}
                for i in value_dict[value]:
                    cwnd[i]=value_dict[value][i]['cwnd_values']
                write_csv(os.path.join(path, CSV_FILE_NAMES[value]), cwnd, compression=compression)
            elif value == 'wireless_data':
                if value_dict[value] is not None:
                    with open('wireless_data.csv', 'w', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow(['timestamp', 'rsrp', 'bler' , 'mcs'])

                        for i in range(len(value_dict[value][0])):
                            writer.writerow([value_dict[value][0][i], value_dict[value][1][i], value_dict[value][2][i], value_dict[value][3][i]])
                        # writer.writerow(value_dict[value])
            else:
